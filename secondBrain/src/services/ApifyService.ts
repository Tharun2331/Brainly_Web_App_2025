...
export class ApifyService {
  ...
  static async extractYouTubeTranscript(videoUrl: string): Promise<{
    ...
    try {
      ...
      // Extract subtitles from result - try multiple fields
      let transcript: string = '';
      
      // Try different subtitle fields
      if (result.subtitles) {
      ...
      // Try alternative subtitle fields
      if (!transcript && result.captions) {
        if (Array.isArray(result.captions)) {
          transcript = result.captions.map((s: any) => s.text || '').join(' ');
        } else if (typeof result.captions === 'string') {
          transcript = result.captions;
        }
      }
      ...
      // If no subtitles found, use description as fallback for Apify
      if (!transcript || transcript.length < 50) {
        if (result.description && result.description.length > 100) {
          console.log('⚠️ No subtitles found, using video description as content');
          transcript = result.description;
        } else {
          throw new Error('No subtitles or description found in Apify response - will try free library');
        }
      }
